...
# In practice, each row represent a shut for an asset with an MSFcode and a shut startdate and enddate, and
#  each row can have additional columns such as index and MSFcode and other columns, and could have : index, asset, MSFcode, startdate, enddate :
# - index is unique
# - multiple assets
...
def split_shuts_periods(df):
    # Input data is df with at least 3 cols : asset, startdate, enddate
    ...
    for split_start, split_end in zip(
        ...
        for _, original_row in active_rows.iterrows():
            ...
            new_row['Originalenddate'] = original_row[startdate]
